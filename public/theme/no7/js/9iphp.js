...
~function (f) {
    var c = f(window), g = f(document), b = f("<div id=ph_lay/>"), a = f("<div id=ph_zoom/>"), h = b.add(a), d = function (j, i, l, k) {
        this.opt = i;
        this.idx = l;
        this.all = k;
        this.len = k.length;
        this.end = this.len > l + 1;
        this.img = f("img:first", j);
        this.lnk = j.addClass("phzoom").unbind("click").bind(this.imgFn()).append(this.hov = f("<span class=ph_hover/>").hide())[0];
        this.cap = f("<div/>", {
            css: {color: i.capColor},
            id: "ph_cap",
            html: f([f("<span/>", {
                id: "ph_txt",
                text: this.img[0].title || this.lnk.title || "No title"
            })[0], f("<span/>", {id: "ph_idx", text: l + 1 + " / " + this.len})[0]])
        }).add(this.nav = f("<div/>", {
            id: "ph_nav",
            css: {color: i.navColor},
            html: (l ? "<span id=ph_prev>" + i.prevText + "</span>" : "") + (this.end ? "<span id=ph_next>" + i.nextText + "</span>" : "")
        }));
        h.click(f.proxy(this, "imgQuit"));
        window.XMLHttpRequest || j.height(this.img.height())
    };
    d.prototype = {
        imgFn: function () {
            var j = this, i = function () {
                return j.hov.not(".loading").stop(0, 1)
            };
            return {
                mouseover: function () {
                    i().fadeIn()
            ...
            var j = this.img, i = c.scrollLeft(), l = c.scrollTop(), n = [c.width(), c.height(), j.width(), j.height(), j.offset().left, j.offset().top];
            this.opt.limitWidth && k > n[0] && (m = m / k * (k = n[0]));
            return n.concat(k, m, (n[0] - k) / 2 + i, (n[1] - m) / 2 + l, (n[0] - n[2]) / 2 + i, (n[1] - n[3]) / 2 + l)
        }, imgLoad: function () {
            b.fadeTo(this.opt.layDur, this.opt.layOpacity);
            var i = this, j = new Image;
            this.hov.addClass("loading");
            j.className = "zoomed";
            j.onload = function () {
                j.onload = null;
                i.hov.hasClass("loading") && (a.height(g.height()).append(j).show(), i.imgAnim(j), i.preLoad())
            };
            j.src = this.lnk.href
        }, imgAnim: function (n) {
            var k = this, j = f(n), m = this.imgPos(n.width || +j.attr("width"), n.height || +j.attr("height")), i = m[0] < m[6], l = this.evtMon(j, m[0], m[0] - m[6], !i);
            j.after(this.cap.hide()).css({left: m[4], top: m[5], width: m[2], height: m[3]}).animate({
                left: m[10],
                top: m[11]
            }, this.opt.animDurA, function () {
                j.animate({left: m[8], top: m[9], width: m[6], height: m[7]}, k.opt.animDurB, function () {
                    k.hov.removeClass("loading");
                    k.cap.css({top: m[7] + m[9], left: i ? 0 : m[8], width: i ? m[0] : m[6]}).fadeTo(300, 0.7);
                    k.nav.bind(l).css("top", m[7] / 3 + m[9]);
                    k.keyBind()
                }).bind(l)
            })
        }, imgQuit: function (i) {
            this.hov.hide().hasClass("loading") ? this.hov.removeClass("loading") : g.unbind(".phzoom");
            i && b.fadeOut();
            a.hide().empty();
            return false
        }, imgChange: function (i) {
            this.imgQuit();
            f(".ph_hover", f(this.all[this.idx + i]).click()).show();
            return false
        }, preLoad: function (i, j) {
            this.idx && (i = new Image, i.src = this.all[this.idx - 1].href, delete i);
            this.end && (j = new Image, j.src = this.all[this.idx + 1].href, delete j)
        }, keyBind: function () {
            var i = this;
            g.bind("keydown.phzoom", function (j) {
                j = j.which;
                return j == 37 && i.idx ? i.imgChange(-1) : j == 39 && i.end ? i.imgChange(1) : j ^ 27 || i.imgQuit(1)
            })
        }, evtMon: function (k, j, i, n) {
            var l = this, m = f("span", this.nav).hide();
            return {
                click: function (o) {
                    return l.len < 2 || (o = o.pageX > j / 2, l.idx ? l.end ? l.imgChange(o || -1) : o || l.imgChange(-1) : !o || l.imgChange(1))
                }, mouseout: function () {
                    m.hide()
                }, mousemove: function (p, o) {
                    p = p.pageX, o = p > j / 2;
                    l.idx ? (m.eq(o).show(), m.eq(1 - o).hide()) : m[o ? "show" : "hide"]();
                    n || (p = p < j / 3 ? 0 : p > j * 2 / 3 ? i : i / 2) == k.position().left || k.not(":animated").animate({left: p}, 200)
                }
            }
        }
    };
    f.phzoom = function (j, i, k) {
        i = f.extend({
            layOpacity: 0.7,
            layDur: 300,
            animDurA: 300,
            ...
            limitWidth: false,
            returnOrigin: true
        }, i), (k = j.has("img"))[0] && (f("#ph_lay")[0] || f("body").append(h), k.each(function (m, l) {
            f.data(l, "phzoom", new d(f(l), i, m, k))
        }));
        return i.returnOrigin ? j : k
    };
    f.fn.phzoom = function (i) {
        return f.phzoom(this, i)
    }
}(jQuery);
// prettify.min.js
!function () {
    ...
    (function () {
        function S(a) {
            ...
            function b(e) {
                var b = e.substring(1, e.length - 1).match(/\\u[\dA-Fa-f]{4}|\\x[\dA-Fa-f]{2}|\\[0-3][0-7]{0,2}|\\[0-7]{1,2}|\\[\S\s]|[^\\]/g), e = [], a = b[0] === "^", c = ["["];
                a && c.push("^");
            ...
            for (var r = {b: 8, t: 9, n: 10, v: 11, f: 12, r: 13}, n = [], k = 0, c = a.length; k < c; ++k) {
                i = a[k];
        ...
        var D = window, y = ["break,continue,do,else,for,if,return,while"], E = [[y, "auto,case,char,const,default,double,enum,extern,float,goto,inline,int,long,register,short,signed,sizeof,static,struct,switch,typedef,union,unsigned,void,volatile"], "catch,class,delete,false,import,new,operator,private,protected,public,this,throw,true,try,typeof"], M = [E, "alignof,align_union,asm,axiom,bool,concept,concept_map,const_cast,constexpr,decltype,delegate,dynamic_cast,explicit,export,friend,generic,late_check,mutable,namespace,nullptr,property,reinterpret_cast,static_assert,static_cast,template,typeid,typename,using,virtual,where"], N = [E, "abstract,assert,boolean,byte,extends,final,finally,implements,import,instanceof,interface,null,native,package,strictfp,super,synchronized,throws,transient"], O = [N, "as,base,by,checked,decimal,delegate,descending,dynamic,event,fixed,foreach,from,group,implicit,in,internal,into,is,let,lock,object,out,override,orderby,params,partial,readonly,ref,sbyte,sealed,stackalloc,string,select,uint,ulong,unchecked,unsafe,ushort,var,virtual,where"], E = [E, "debugger,eval,export,function,get,null,set,undefined,var,with,Infinity,NaN"], P = [y, "and,as,assert,class,def,del,elif,except,exec,finally,from,global,import,in,is,lambda,nonlocal,not,or,pass,print,raise,try,with,yield,False,True,None"], Q = [y, "alias,and,begin,case,class,def,defined,elsif,end,ensure,false,in,module,next,nil,not,or,redo,rescue,retry,self,super,then,true,undef,unless,until,when,yield,BEGIN,END"], W = [y, "as,assert,const,copy,drop,enum,extern,fail,false,fn,impl,let,log,loop,match,mod,move,mut,priv,pub,pure,ref,self,static,struct,true,trait,type,unsafe,use"], y = [y, "case,done,elif,esac,eval,fi,function,in,local,set,then,until"], R = /^(DIR|FILE|vector|(de|priority_)?queue|list|stack|(const_)?iterator|(multi)?(set|map)|bitset|u?(int|float)\d*)\b/, V = /\S/, X = v({
        ...
        p(v({keywords: Q, hashComments: !0, multiLineStrings: !0, regexLiterals: !0}), ["rb", "ruby"]);
...
!function (a) {
    ...
    return function a(b, c, d) {
        ...
        1: [function (a, b, c) {
        ...
        }, {closest: 2, "component-event": 4}], 2: [function (a, b, c) {
        ...
        }, {"matches-selector": 3}], 3: [function (a, b, c) {
            function d(a, b) {
            ...
            d.prototype = {
                on: function (a, b, c) {
                    ...
                    if (d && b)for (var f = 0, g = d.length; g > f; f++)d[f].fn !== b && e.push(d[f]);
            ...
            var e = function () {
                ...
                function a(b) {
                    if (d(this, a), this.action = b.action, this.emitter = b.emitter, this.target = b.target, this.text = b.text, this.trigger = b.trigger, this.selectedText = "", this.text && this.target)throw new Error('Multiple attributes declared, use either "target" or "text"');
                ...
                return a.prototype.selectFake = function () {
                    var a = this;
                    ...
                    key: "action", set: function (a) {
                        if (this._action = a || "copy", "copy" !== this._action && "cut" !== this._action)throw new Error('Invalid "action" value, use either "copy" or "cut"')
                    }, get: function () {
                        return this._action
                    }
                }, {
            ...
            var g = a("./clipboard-action"), h = d(g), i = a("delegate-events"), j = d(i), k = a("tiny-emitter"), l = d(k), m = "data-clipboard-", n = function (a) {
                function b(c, d) {
                    var f = this;
                    e(this, b), a.call(this), this.resolveOptions(d), j["default"].bind(document.body, c, "click", function (a) {
                        return f.initialize(a)
                    })
                }

                return f(b, a), b.prototype.resolveOptions = function (a) {
                    a = a || {}, this.action = "function" == typeof a.action ? a.action : this.setAction, this.target = "function" == typeof a.target ? a.target : this.setTarget, this.text = "function" == typeof a.text ? a.text : this.setText
                }, b.prototype.setAction = function (a) {
                    return a.getAttribute(m + "action")
...
function doPjax() {
    ...
    if ($('pre').hasClass('php')) {
        $('pre').parent('div.code').attr('lang', 'PHP');
    } else if ($('pre').hasClass('html')) {
        $('pre').parent('div.code').attr('lang', 'HTML');
    } else if ($('pre').hasClass('js')) {
        $('pre').parent('div.code').attr('lang', 'JS');
    } else if ($('pre').hasClass('css')) {
        $('pre').parent('div.code').attr('lang', 'CSS');
    } else if ($('pre').hasClass('shell')) {
        $('pre').parent('div.code').attr('lang', 'SHELL');
    } else {
        $('pre').parent('div.code').attr('lang', 'Unknown');
    }
    ...
    if ($('#sidebar').length > 0 && $("#main").height() > $("#sidebar").height()) {
        $('#sidebar').affix({
            offset: {
                ...
                bottom: $('footer#body-footer').outerHeight(true) + $('.main-footer').outerHeight(true) + 25
            }
        });
    }
}
